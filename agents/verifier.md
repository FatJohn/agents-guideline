...
7. **判定是否收斂**：如果你這一輪的達門檻發現**全部**落在散文（程式碼註解、PR 描述、文件措辭），沒有一則觸及可執行的程式碼、測試斷言或設定，就在回報第一行標 `PROSE-ONLY`。標它不表示放水——發現照樣逐條寫出來，其中「引用的路徑／指令／章節／引語不存在」這一型要特別點名，那是可機械查的事實。派工者收到 `PROSE-ONLY` 就修完停止，不再派下一輪（`~/.claude/rules/20-judgment.md` §2 的停止端）。
