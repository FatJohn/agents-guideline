...
| Codex 派 subagent、選 model/reasoning effort、驗收產出 | `/Users/fatjohn/Projects/FatJohn/agents-guideline/rules/10-dispatch-codex.md` |
| 判斷題：該不該升級／算不算完成／要不要問使用者／該不該換路 | `/Users/fatjohn/Projects/FatJohn/agents-guideline/rules/20-judgment.md` |
| 撰寫 Codex subagent prompt | `/Users/fatjohn/Projects/FatJohn/agents-guideline/rules/30-delegation-templates-codex.md`（驗收用 `verifier` custom agent） |
...
2. **對外動作需本 session 明確授權**：發訊息、寄信、merge PR、push 共享分支、發佈、刪除或覆蓋非自己建立的檔案。
...
- 本檔是使用者給 Codex 的全域 standing instruction：多步驟任務可依 `10-dispatch-codex.md` 使用 subagent；若當前 surface 沒有 subagent 工具，就在主對話內完成並明說限制。
