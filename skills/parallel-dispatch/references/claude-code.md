...
| 契約項目 | 本 adapter 的做法 |
|---|---|
| launch | 一個 controller session 在**同一則訊息**發多個 `Agent` 呼叫，每片一個；實作用 `worker`（不帶 `model`，由 frontmatter 決定；Sonnet 備用為 `worker-sonnet`，顯式 `model: sonnet`，見 `~/.claude/rules/10-dispatch.md` §0），切片與整合驗收用 fresh `verifier`（`model: opus`）。subagent 在 controller session 內跑，沒有獨立 terminal |
