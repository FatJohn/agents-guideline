...
本 skill 只管 **orchestration strategy**：值不值得拆、怎麼拆、誰擁有什麼、怎麼驗、怎麼整合。它**不是** workspace／terminal／process manager。`<REPO>` 依全域 `CLAUDE.md`／`AGENTS.md` 的 bootstrap 解出；本檔承接 Claude `<REPO>/rules/10-dispatch.md` 與 Codex `<REPO>/codex/rules/10-dispatch-codex.md`，是切分與整合的 canonical；驗收沿用 rules。
...
**改派單一 worker**（任一成立）：改動範圍小或多數集中在同一檔；任一片沒有獨立驗收條件；只有一片無依賴（其餘都在等它）；每片都會碰同一函式或相鄰區段（交由 §3 重疊矩陣判定）；整份 spec 單一實作者即可完成，拆了不到一個 agent 的合理工作量。相同檔但不同且不相鄰的函式不算 git 重疊，仍受 §3 語意風險檢查。

**選 pattern**：
- **Pattern A — Divide and Conquer**：重點是 ownership、dependency、integration order。這是預設。
...
- **brief 只描述 WHAT**：branch 名、commit 訊息、push／PR 等 commit-time 事項只寫在 brief 的 Execution environment 欄（由 adapter 填），Objective 與 Expected output 不得提到，避免兩層指示互相打架、worker 解衝突的方式不一致。
...
**Agent 選擇**：caller 可指定 preferred agent、available agents、agent capabilities；沒指定就用當前 adapter 角色表的預設 worker。可依任務性質建議（實作、refactor、research、測試、review 各有偏好）。
...
3. 計畫裡點名的符號、路徑、訊息逐項 `git grep`，零命中＝被默默丟掉；grep 只證存在不證正確，正確性靠 §5 與第 7 步。
...
| worker 停滯（還在跑，但落地位置的 `git status --short`／`diff --stat` 兩次 read-back 無變化；**第一次 Edit 前的探索段不計**） | 進度以 git 可見改動為準，不以 log 或輸出活動為準；探索段＝落地位置出現第一個 git 改動之前，這段不判停滯（能查狀態的 adapter 可用活動訊號確認它還活著，但活動不算進度）；兩次 read-back 的間隔門檻未實測、依經驗調整（理由見 `<REPO>/docs/judgment-rationale.md`「2026-09-22 parallel-dispatch skill fable review 落地」段）；stop 該 worker，補脈絡一次或換 fresh context |
...
| 測試失敗 | 屬該片 → 退回修正，依 §6 第 10 步判斷 fresh／continue；重試上限見下方「re-plan 規則」；屬 base 或其他片 → 回 §3 判斷依賴是否變了 |
...
**Adapter 契約**（launch／follow-up／query status／collect／stop 五件事、workspace 建法、角色名稱與 model 對應表、不能做的邊界）完整定義見 `<REPO>/docs/parallel-dispatch-design.md`「Adapter 契約」。
...
**刻意不做（屬 framework 層，不進本 skill）**：清單與理由見 `<REPO>/docs/parallel-dispatch-design.md`「刻意不做」。
