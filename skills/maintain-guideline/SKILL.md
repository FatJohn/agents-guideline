...
- **`<REPO>/rules/20-judgment.md` §2 的「補充判準」硬上限 9 條**（計數方式：該節內以 `**補充判準（` 開頭的區塊數，2026-09-06 實測為 9，即目前正好滿額；標「停止端」的區塊不計入）：要加第 10 條，**必須先合併或整條日落／搬走一條**，不准「先加了再說」。**不算違規**：把既有兩條合併成一條後總數仍 ≤9；整條搬走且不留無例骨架（見上方「不准為了省 bytes 刪掉常駐判準的正反例」那條）後空出的名額。
