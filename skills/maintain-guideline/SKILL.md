...
- 觸發：`<REPO>/rules/50-lessons.md` 的活躍教訓超過 **8** 條，或任一 `<REPO>/rules/` 檔超過 200 行，或 `<REPO>/rules/` 全目錄超過 40,000 bytes。行／byte 是 Claude 常駐內容的代理量，不等於 token。Codex 的共用 rules 與 codex/rules 按需載入，另計實際載入內容。
