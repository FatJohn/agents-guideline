...
- **上面的 200 行、20,000／44,000 bytes 是觸發審查的 tripwire，不是要逼近的優化目標**：碰到門檻就排一次日落審查，不是「砍到剛好低於門檻」就算處理完；沒碰到門檻也不代表不能瘦身。（活躍教訓那 8 條是硬上限，見下方「活躍教訓是硬上限 8 條」那條，不適用本句。）
  ...
  調整理由（2026-09-09）：近 50 天 39 個動到 `rules/` 的 commit，min 35,450／median 38,253／max 43,530，40,000 響了 5 次、多數是雜訊；44,000 只在真成長時響。加每檔上限是因為 `<REPO>/rules/20-judgment.md` 單檔已占全目錄 47.9%、其 §2 一節占 31.1%，目錄總量看不到哪個檔在長。
...
- **`<REPO>/rules/20-judgment.md` §2 的「補充判準」硬上限 9 條**（計數方式：該節內以 `**補充判準（` 開頭的區塊數；2026-09-06 當時實測為 9，2026-09-07 合併修正／停止端／輪次後為 8；標「停止端」的區塊不計入）：要加第 10 條，**必須先合併或整條日落／搬走一條**，不准「先加了再說」。**不算違規**：把既有兩條合併成一條後總數仍 ≤9；整條搬走且不留無例骨架（見上方「不准為了省 bytes 刪掉常駐判準的正反例」那條）後空出的名額。
  訂這條的理由（2026-09-06）：§2 在一次 session 內長了 3,459 bytes（`## 2.` 到 `## 3.` 之間，含不含標題行兩種量法的差值相同），占 `<REPO>/rules/` 全預算超過四分之一。形狀與 `<REPO>/rules/50-lessons.md` 訂上限之前完全一樣——每條單獨看都對、只進不出，而逐條檢查後也沒有一條適合搬去 docs（它們的共同形狀是「觸發訊號認得出來，但該做什麼不會自己想到」，搬走等於指望人在不知道要查的時候去查）。既然搬不動，唯一有效的機制就跟 50-lessons 一樣：逼人在加新條目時付出清舊條目的成本。
...
- **常駐判準的格式（2026-09-09）**：`<REPO>/rules/20-judgment.md` 每條判準／補充判準的正文**四句以內、約 400 bytes**——觸發情境、要做什麼、不算違規、指向（可省）——加一組 ✅／❌。訂條理由、事故脈絡、修辭句、與鄰條的分工說明一律寫進 `<REPO>/docs/judgment-rationale.md` 對應標題下（原句搬入，不改寫），常駐區不留。依本條把理由句搬出常駐正文屬「搬移」，不算下方「整檔重寫」；首次全檔套用已於 2026-09-09 經使用者同意。**400 bytes 是目標不是門檻**：機制事實、指令、數字與「不算違規」邊界砍不動而超標**不算違規**，回報實際 bytes 即可——砍到邊界句才是違規。
  訂這條的理由：2026-09-09 量得判準正文占該檔 71%，其中 28% 是理由／分工／事故日期，而執行者只需要判準與邊界；壓縮的對象是修辭與理由，機制事實與正反例一項不少，才不會退化成「只剩動詞與檔名」的指向散文。
