...
- 平行派工成本量測（2026-09-17，對 09-07 起 75 個 Claude Code session、229 個 verifier、137 個 worker 的 jsonl 統計；成本以 opus 相對價權重 input 1／cache 寫 1.25／cache 讀 0.1／output 5 計）：單輪 worker 中位 58 次 request（≈工具呼叫數＋1，完成通知 `<usage>` 的 `tool_uses` 即此數）；被 `SendMessage` 續用修 finding 的 71 個 worker 占 worker 總成本 82%，最大滾到 500 次 request／700K context（起始 36K）。verifier 首輪 OPEN 率 64%，FAIL 以可證偽宣稱（數字、封閉量詞、行號、註解／測試名的行為宣稱）居首、測試／lint 次之；按輪次算首輪成本 78M、第 2 輪以後 46M。有 ≥3 個 subagent 的 session 活躍時間：worker 在跑 44%、只有 verifier 在跑 18%、主對話自己 37%；主對話 context 中位 337K、占總成本 67%。量法：`~/.claude/projects/<proj>/<sid>/subagents/agent-*.jsonl`＋同名 `.meta.json`（agentType／model），usage 以 requestId 去重。
