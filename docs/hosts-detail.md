...
- **symlink／rules 載入探測（2026-09-20 重驗，結論與 2026-08-05 相反）**：本機**開啟**任何 reparse point 背後的檔案都會失敗，錯誤是 `ERROR_UNTRUSTED_MOUNT_POINT`（os error 448）。
  - 實測矩陣（目錄 symlink 與 junction、C:→C: 與 C:→E:、新建與 2026-08-05 舊建，全部涵蓋）：**開檔一律失敗**。唯一的例外是 **Git Bash（MSYS）自行解析 reparse data**——它既讀得到檔案 symlink（`head -1 ~/.claude/CLAUDE.md` 成功），也列得出目錄 symlink 的內容（`ls ~/.claude/rules` 回五個檔名），但 `cat ~/.claude/rules/00-environment.md` 仍然 Permission denied。
