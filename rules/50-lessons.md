...
- [2026-07-25][flutter-slimgo] 見 `.agents/skills/` 與 `.claude/skills/` 同名 SKILL.md `diff -q` 顯示 differ，就以「兩份漂移了」為前提問使用者要不要同步；拿到「把新版同步過去」的授權後才實際 diff，發現 `.agents` 版是刻意為 Codex 寫的英文精簡版（86 vs 175 行，另有 `agents/openai.yaml`），整份覆蓋會毀掉它 → 拿「兩個檔案不一致」去問使用者之前，先看清不一致的**性質**（漂移事故 vs 刻意分版），否則拿到的是基於錯誤前提的授權；授權到手後動手前仍要複查前提，前提錯了就回報並改做符合意圖的最小正確動作 → 已套用到：尚未（再踩就提案入 20-judgment §3）
- [2026-07-25][macroeconomics-report] 瘦身專案 CLAUDE.md 時把「檔 < 300、函式 < 50」改寫成「eslint 有規則機械化把關、超了 lint 會紅」以強化說服力，fable-verifier 抓到：`max-lines` 確實是 error 300，但 `max-lines-per-function` 是 **warn 80**（該 repo 2026-05-20 明文否決 50/error），且 lint script 是裸 `eslint .` 無 `--max-warnings 0`，warn 根本不會失敗 → 把「有 lint／hook／CI 擋」寫進規則前先查三件事：rule 的 severity、閾值實際數字、跑它的 script 有沒有讓 warning 變失敗；假的機械化宣稱比沒寫更糟（後人會信任它而不自查，或「順手修好」config 而推翻刻意決策）→ 已套用到：尚未（再踩就提案入 rubrics/document-quality.md）
