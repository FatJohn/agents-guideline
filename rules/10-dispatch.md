...
- **「第二個現場」的答案是取樣不是清單，先按派工範圍分流**：範圍內的照 `20-judgment.md` §2 停止端修並重驗 delta；範圍外的不修，**登記到該專案 repo 內會被回頭讀的位置**（issue、既有待辦檔、或派工文件的「殘留」章節，沒有就開一個），回報附路徑或連結——只寫在對話裡等於沒登記。**連續兩輪的 FAIL 都落在範圍外，代表你在替下一個批次做事**：停下來回報。與停止端的分工：那條管「這則發現要不要重驗 delta」，本條管「這則發現該不該由這次工作處理」。
  - ✅ **正例**：verifier 指出檢查器還有三種字串間接寫法沒擋，其中兩種要先有 allowlist 決策 → 修範圍內那一種並重驗 delta，另兩種登記進派工文件的殘留章節、回報附路徑。
  - ❌ **反例**：連續十輪都在補下一種寫法，每輪的 FAIL 都是上一輪修正帶出來的新誤叫或新繞過，而派工文件的驗收條件從第一輪起就全數通過。
