...
> **`<REPO>` 對照**（canonical；新機器要加）：主力 Mac（`xushengzhedeMacBook-Pro.local`）`~/Projects/FatJohn/agents-guideline` → `hosts/macos.md`；Windows 桌機（`FatJohn-PC`）`E:\agents-guideline` → `hosts/windows.md`。
