...
**判準**：符合任一條就升級（本節是升級門檻的 canonical 定義；門檻成立後派給誰——Claude：`../docs/escalation-paths.md`；Codex：`../codex/rules/10-dispatch-codex.md` §5「升降級路徑」）：
...
註：已在高能力層時，觸發升級的形式是換 fresh context、取得第二意見或重新定義問題，不只是換一套措辭重試。具體選項見 Claude：`../docs/escalation-paths.md`；Codex：`../codex/rules/10-dispatch-codex.md` §5「升降級路徑」。
...
**停止端（與上一條配套，缺了它會無限遞迴）**：驗收的對象是程式碼、測試與設定。**一輪的達門檻
發現若全部落在散文**（註解、PR 描述、判準文件），沒有一則觸及可執行的東西，就修完、把剩餘
登記下來，**不再派下一輪**。理由：程式碼才是真正的文件，註解本來就會過時；散文修正不會讓
任何閘門變紅，所以下一輪必然還找得到措辭問題，那個迴圈沒有終點。**發現曾觸及程式碼的輪次
不適用**——那代表產出本身還在動，照上一條重驗 delta。**唯一例外是引用真實性**（檔案路徑、
指令、章節、引語是否真的存在）：那是可機械查的事實、不是措辭品味，發現了就修，但同樣不另開
一輪。verifier 端的對應動作見 `~/.claude/agents/verifier.md` 第 7 條（回報標 `PROSE-ONLY`）。
