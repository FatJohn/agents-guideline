...
- 本系統 repo：`E:\agents-guideline`。**全域設定是 `../scripts/sync-profile.py` 同步過去的實體檔複本**（2026-09-20 起，`~/.claude`／`~/.codex`／`~/.agents` 三處；Codex agent TOML 另由 `sync-codex-agents.py` 管）；正解是用 admin 建 symlink，下次重建時改回去。
  - **改了 repo 不會自動生效**——動完 `rules/`、`hosts/`、`rubrics/`、`skills/`、`agents/`、`CLAUDE.md`、`AGENTS.md` 要跑 `python "E:/agents-guideline/scripts/sync-profile.py" --apply --update`（加 `--prune` 清掉已 sunset 的舊項），否則你讀到的規則和跑起來的規則會不一樣。
...
- **Claude Code 的 Bash 工具會先把指令字串裡的 `\\` 收成 `\`（單引號與 heredoc 內都一樣，2026-09-20 實測 `printf '%s' 'x\\y'` 印出 `x\y`；Mac 同日實測同指令印出 `x\\y`，無此現象）**：含反斜線的 regex／python 片段用 Write 寫成檔再執行，或用 Edit 工具取代 sed。
